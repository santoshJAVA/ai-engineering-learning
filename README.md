...
By using Pydantic, I learned how to force the LLM to return strict, perfectly formatted JSON. robust_extractor.py also adds tenacity for exponential backoff (retry logic) because APIs are flaky.

Run it: python robust_extractor.py
...
Run it: python full_rag_pipeline.py

3. Tool Calling (agent.py)

The Epiphany: LLMs can actually do things.
This script gives the OpenAI model the schema for a local Python function (create_linear_ticket). The AI realizes it needs to use the tool, pauses text generation, passes the JSON arguments to Python, and waits for the execution result.
